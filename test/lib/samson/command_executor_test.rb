...
describe Samson::CommandExecutor do
  describe "#execute" do
    it "runs" do
      Samson::CommandExecutor.execute("echo", "hello", timeout: 1).
        must_equal OpenStruct.new(status: true, error: "", output: "hello\n")
    end

    it "captures stderr" do
      Samson::CommandExecutor.execute("sh", "-c", "echo hello 1>&2", timeout: 1).
        must_equal OpenStruct.new(status: true, error: "hello\n", output: "")
    end
    ...
    it "fails nicely on missing exectable" do
      Samson::CommandExecutor.execute("foo", "bar", timeout: 1).
        must_equal OpenStruct.new(status: false, error: "No such file or directory - foo", output: "")
    end

    it "does not fail on nil commands" do
      Samson::CommandExecutor.execute("echo", 1, nil, timeout: 1).
        must_equal OpenStruct.new(status: true, error: "", output: "1 \n")
    end
    ...
    it "times out and cleans up" do
      ...
      time = Benchmark.realtime do
        Samson::CommandExecutor.execute(*command, timeout: 0.1).
          must_equal OpenStruct.new(status: false, error: "execution expired", output: "")
      end
      time.must_be :<, 0.2
    ...
    it "does not fail when pid was already gone" do
      ...
      Samson::CommandExecutor.execute("sleep", "0.2", timeout: 0.1).
        must_equal OpenStruct.new(status: false, error: "execution expired", output: "")
    ...
    it "waits for zombie processes" do
      ...
      time = Benchmark.realtime do
        Samson::CommandExecutor.execute("sleep", "0.5", timeout: 0.1).
          must_equal OpenStruct.new(status: false, error: "execution expired", output: "")
      end
      time.must_be :>, 0.5
    end
    ...
    it "does not allow injection" do
      Samson::CommandExecutor.execute("echo", "hel << lo | ;", timeout: 1).
        must_equal OpenStruct.new(status: true, output: "hel << lo | ;\n", error: "")
    end

    it "does not allow env access" do
      with_env FOO: 'bar' do
        Samson::CommandExecutor.execute("printenv", "FOO", timeout: 1).
          must_equal OpenStruct.new(status: false, error: "", output: "")
      end
    end

    it "can set env" do
      Samson::CommandExecutor.execute("printenv", "FOO", timeout: 1, env: {"FOO" => "baz"}).
        must_equal OpenStruct.new(status: true, error: "", output: "baz\n")
    end
