...
describe "Samson::FormBuilder Helper" do
  def fake_erb_rendering
    builder.instance_variable_get(:@template).output_buffer << "XYZ"
  end

  let(:object) { User.new }
  let(:builder) { Samson::FormBuilder.new(:user, object, self, {}) }
  ...
  describe '#actions' do
    ...
    it "can include delete link" do
      expects(:url_for).with(builder.object).returns('/xxx')
      builder.actions(delete: true).must_include "Delete"
    end

    it "can include custom delete link" do
      expects(:url_for).with([:admin, commands(:echo)]).returns('/xxx')
  ...
  describe '#fields_for_many' do
    ...
    let(:builder) { Samson::FormBuilder.new(:project, project, self, {}) }
