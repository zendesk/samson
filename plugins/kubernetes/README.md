...
If `metadata.labels.kubernetes.io/cluster-service: 'true'` is set, it will keep `default` or `kube-system`. 
If `metadata.annotations.samson/keep_namespace: 'true'` is set, it will keep anything.
