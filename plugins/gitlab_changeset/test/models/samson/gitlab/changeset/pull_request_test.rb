...
describe Samson::Gitlab::Changeset::PullRequest do
  def add_risks
    body.replace(+<<~BODY)
      # Risks
       - Explosions
  ...
  describe ".find" do
    it "finds the pull request" do
      ...
      pr = Samson::Gitlab::Changeset::PullRequest.find("foo/bar", 42)
  ...
  describe ".changeset_from_webhook" do
    it 'finds the pull request' do
      ...
      pr = Samson::Gitlab::Changeset::PullRequest.changeset_from_webhook(project, params)
  ...
  describe ".valid_webhook?" do
    ...
    it "is invalid for PRs that had its label changed" do
      ...
      Samson::Gitlab::Changeset::PullRequest.valid_webhook?(webhook_data).must_equal false
    end
    ...
    describe "PR change that is an edit" do
      ...
      it 'is valid if [samson review] was not in the previous description' do
        ...
        Samson::Gitlab::Changeset::PullRequest.valid_webhook?(webhook_data).must_equal true
      end
  ...
  describe "#title_without_jira" do
    before do
      GITHUB.stubs(:pull_request).with("foo/bar", 42).returns(data)
      Samson::Gitlab::Changeset::PullRequest.find("foo/bar", 42)
    end
  ...
  describe "#jira_issues" do
    ...
    let(:pr_no_body) { Samson::Gitlab::Changeset::PullRequest.new("xxx", data_nil_body) }
    ...
    it "returns a list of JIRA issues referenced in the PR body" do
      ...
      pr.jira_issues.must_equal [
        Samson::Gitlab::Changeset::JiraIssue.new("https://foobar.atlassian.net/browse/XY-123"),
        Samson::Gitlab::Changeset::JiraIssue.new("https://foobar.atlassian.net/browse/AB-666")
      ]
    end
    ...
    it "returns a list of JIRA urls using JIRA_BASE_URL ENV var given JIRA codes" do
      with_env JIRA_BASE_URL: 'https://foo.atlassian.net/browse/' do
        ...
        pr.jira_issues.must_equal [
          Samson::Gitlab::Changeset::JiraIssue.new("https://foo.atlassian.net/browse/XY-123"),
          Samson::Gitlab::Changeset::JiraIssue.new("https://foo.atlassian.net/browse/AB-666")
        ]
      end
    end
    ...
    single_key_cases.each do |casebody, key|
      it "returns #{key} when given \"#{casebody}\"" do
        with_env JIRA_BASE_URL: jira_url do
          ...
          pr.jira_issues.must_equal [Samson::Gitlab::Changeset::JiraIssue.new(full_url)]
        end
      end
    end
    ...
    multiple_keys_cases.each do |casebody, keys|
      it "returns #{keys} when given \"#{casebody}\"" do
        with_env JIRA_BASE_URL: jira_url do
          body.replace(casebody)
          pr.jira_issues.must_equal(keys.map { |x| Samson::Gitlab::Changeset::JiraIssue.new(jira_url + x) })
        end
      end
    end
