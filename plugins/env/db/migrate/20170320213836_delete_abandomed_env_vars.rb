...
class DeleteAbandomedEnvVars < ActiveRecord::Migration[5.0]
  ...
  def up
    ...
    [Project, EnvironmentVariableGroup].each do |klass|
      ...
      write "DELETED #{deleted} #{klass.name} EnvironmentVariables by parent"
    end
    ...
    [DeployGroup, Environment].each do |klass|
      ...
      write "DELETED #{deleted} #{klass.name} EnvironmentVariables by scope"
    end
    ...
    write "DELETED #{deleted} ProjectEnvironmentVariableGroup by project"
    ...
    write "DELETED #{deleted} ProjectEnvironmentVariableGroup by environment_variable_group"
  end
