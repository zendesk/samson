...
describe Samson::Github::Changeset::GithubUser do
  let(:user) do
    Samson::Github::Changeset::GithubUser.new(
      stub(
        "data",
  ...
  let(:unknown_user) { Samson::Github::Changeset::GithubUser.new(stub("data", login: nil, avatar_url: nil)) }
  ...
  describe "#eql?" do
    it "is equal if the login matches" do
      other = Samson::Github::Changeset::GithubUser.new(stub("data", login: "foo"))
      user.eql?(other).must_equal true
    end

    it "is not equal if the login does not match" do
      other = Samson::Github::Changeset::GithubUser.new(stub("data", login: "bar "))
      user.eql?(other).must_equal false
    end

    it "is equal if both users are unknown" do
      other = Samson::Github::Changeset::GithubUser.new(stub("data", login: nil))
