...
describe Samson::Github::Changeset::Commit do
  let(:commit_data) { stub }
  let(:data) { stub("data", commit: commit_data, author: stub, sha: "aa2a33444343") }
  let(:commit) { Samson::Github::Changeset::Commit.new("foo/bar", data) }
  ...
  describe "#author" do
    it "returns a github user if there is an author" do
      commit.author.class.must_equal Samson::Github::Changeset::GithubUser
    end
  ...
  describe 'status' do
    it 'returns a valid status' do
      status = {success: 'true'}
      GITHUB.stubs(:combined_status).returns([[:success, 'true']])
      Samson::Github::Changeset::Commit.status('foo', 'abc123').must_equal status
    end
